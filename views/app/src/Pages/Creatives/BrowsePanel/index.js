...
class BrowsePanel extends PureComponent {
  ...
  handleIframeLoad(e) {
    ...
    const browseRoute = e.target.contentWindow.location.href.slice(
      paths.origin.length
    )
    window.history.replaceState(
      {},
      '',
      this.componentPaths.appRoute + browseRoute
    )
  }
  ...
  render() {
    ...
    return (
      <div className={`browse-panel ${deactivatedClass}`}>
        <iframe
          ref="browse-iframe"
          src={`${this.componentPaths.origin +
            this.componentPaths.browseRoute}`}
