...
function save(profilesJson) {
	try {
		...
		rowLog('Unable to save build/profiles.json', err)
	}
}
...
function getProfileTargetIndex(profile, target) {
	// rowLog('getProfileTargetIndex()', 'profile.targets.length', profile.targets.length)
	for (var i = 0; i < profile.targets.length; i++) {
		// rowLog('getProfileTargetIndex()', `profile.targets[${i}].size`, profile.targets[i].size, `target.size`, target.size, `profile.targets[${i}].index`, profile.targets[i].index, `target.index`, target.index)
...
function markDeployInProgress(profileName, target) {
	...
	// rowLog('markDeployInProgress()', profileName, `${target.size}/${target.index}`)
	let profilesJson = load()
	const i = getProfileTargetIndex(profilesJson[profileName], target)
	// rowLog('markDeployInProgress()', `i`, i)
	profilesJson[profileName].targets[i].deployAt = '...'
	// rowLog('markDeployInProgress()', `profilesJson[${profileName}].targets[${i}].deployAt`, profilesJson[profileName].targets[i].deployAt)
	save(profilesJson)
}
...
function markDeployed(profileName, target) {
	...
	rowLog('markDeployed()', profileName, `${target.size}/${target.index}`)
	let profilesJson = load()
	const i = getProfileTargetIndex(profilesJson[profileName], target)
