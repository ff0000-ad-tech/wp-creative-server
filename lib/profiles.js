...
function getCurrentProfile() {
	...
	return packageJson.currentProfile || defaultProfile
}
// get all profiles
...
function updateProfile(name, profile) {
	...
	packageJson.currentProfile = name
	save(packageJson)
}
...
function markDeployed(name, targets) {
	...
	targets.forEach(target => {
		const i = getProfileTargetIndex(packageJson.profiles[name], target)
		packageJson.profiles[name].targets[i].deployAt = moment(value.updateAt).from(Date.now())
	})
	save(packageJson)
}
