...
function prepareCmd(ctype, params) {
	...
	// webpack command
	const cwd = global.wpDmPath || global.servePath
	const command = 'npx'
	const cmdArgs = ['webpack']
	const args = ['--config', 'webpack.config.js', '--env', JSON.stringify(env), '--colors']

	// escape double-quotes and double-quote args to be compatible with winblows
	const shell = `cd "${cwd}" && ${command} ${cmdArgs.join(' ')} ${cmdEscape(args)}`
