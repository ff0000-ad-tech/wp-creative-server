...
function startWatching(params) {
	const targets = state.getTargets(generateId(params.size, params.index))
	if (!targets) {
		return
	}
	...
	for (var id in targets) {
		...
		const outstream = fs.createReadStream(targets[id].webpack.out, { autoClose: false })
		outstream
			.on('data', data => {
				log('data')
				// update state
